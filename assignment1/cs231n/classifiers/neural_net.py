...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    normalized_scores = exp_scores / (np.sum(exp_scores, axis=1)[:, np.newaxis])
    ...
    softmax_matrix = normalized_scores
    cum_exp_minus_correct_score = np.sum(exp_scores, axis=1) - exp_scores[range(num_train), y]
    softmax_matrix[range(num_train), y] = -1*cum_exp_minus_correct_score/np.sum(exp_scores, axis=1)
    softmax_matrix /= num_train
    
    #two_layer_net gradients
    dW2 = np.dot(h1.T, softmax_matrix)
    db2 = np.sum(softmax_matrix, axis=0)
    
    hidden_layer = np.dot(softmax_matrix, W2.T)
    hidden_layer[h1==0] = 0
